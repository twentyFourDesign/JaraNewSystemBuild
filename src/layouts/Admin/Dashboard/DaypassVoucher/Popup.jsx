...
const Popup = ({setshowPopup,setData}) => {
    const [data, setstaffData] = useState({code:"",amount:"",balance:"",startsAt:"",expires:"",status:"",percentage:""})

    const submitData = ()=>{
        if(!data.code || !data.amount || !data.balance || !data.startsAt || !data.expires || !data.status || !data.percentage){
            toast.error("All Fields Are Required")
        }
        else{
            axios.post(`${baseUrl}/daypass/voucher/create`,data)
            .then((res)=>{
                if(res.status){
  ...
  return (
    ...
    <div className='p-5 bg-white w-[20rem] rounded-md'>
        ...
        <input onChange={(e)=>setstaffData({...data,percentage:e.target.value})} type="text" name="email" id="" placeholder='Percentage' className='w-[100%] h-[2.3rem] px-3 border-2 border-gray-300 mt-4 rounded-md outline-none'/>
        ...
        <input onChange={(e)=>setstaffData({...data,expires:e.target.value})} type="text" name="expireAt" id="" placeholder='Expired At' className='w-[100%] h-[2.3rem] px-3 border-2 border-gray-300 mt-4 rounded-md outline-none'/>
