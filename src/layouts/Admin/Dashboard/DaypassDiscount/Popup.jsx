...
const Popup = ({ setshowPopup, setData }) => {
  ...
  const submitData = () => {
    if (!data.code || !data.percentage || !data.days || !data.expires) {
      toast.error("All Fields Are Required");
    } else {
      axios.post(`${baseUrl}/daypass/discount/create`, data).then((res) => {
        if (res.status) {
          toast.success("Discount Created");
          axios.get(`${baseUrl}/daypass/discount/get`).then((res) => {
            setData(res?.data);
          });
          setTimeout(() => {
            setshowPopup(false);
  ...
  return (
    <div className="p-5 bg-white w-[20rem] rounded-md">
      ...
      />
      <input
        onChange={(e) => setstaffData({ ...data, percentage: e.target.value })}
        type="number"
        name="percentage"
        id=""
      ...
      <div className="flex flex-col mt-4">
        ...
        <input
          onChange={(e) => setstaffData({ ...data, expires: e.target.value })}
          type="date"
          name="expireAt"
          id="expires"
          placeholder="Expires"
      ...
      <input
        onChange={(e) => setstaffData({ ...data, days: e.target.value })}
        type="text"
        name="days"
        id=""
        placeholder="Days"
