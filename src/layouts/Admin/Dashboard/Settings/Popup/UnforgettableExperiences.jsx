...
const UnforgettableExp = ({ setShowPopups, showPopups }) => {
  const [data, setData] = useState({ title: "", price: "" });
  const [apiData, setapiData] = useState([]);

  const getData = async () => {
    let response = await axios.get(`${baseUrl}/unforgettableExp/get`);
  ...
  const create = async () => {
    let response = await axios.post(`${baseUrl}/unforgettableExp/create`, data);
    if (response.status) {
      getData();
      toast.success("unforgettableExp Added");
    }
  };

  const update = async (id) => {
    let response = await axios.put(
      `${baseUrl}/unforgettableExp/update/${id}`,
      data
    );
    if (response.status) {
      getData();
      toast.success("unforgettableExp Updated");
    }
  };

  const del = async (id) => {
    let response = await axios.delete(
      `${baseUrl}/unforgettableExp/delete/${id}`
    );
    if (response.status) {
      // getData()
      toast.success("unforgettableExp Deleted");
    }
  };
    ...
    <div className="bg-white p-3 w-[90%] md:w-[80%] rounded-md overflow-auto h-[40rem]">
      <div className="flex justify-between items-center w-[100%]">
        ...
        <ImCross
          ...
          onClick={() => {
            setShowPopups({ ...showPopups, unforgettableExp: false });
          }}
        />
      </div>
