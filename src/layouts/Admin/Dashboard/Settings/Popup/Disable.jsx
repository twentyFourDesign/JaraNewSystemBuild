...
const Disable = ({ setShowPopups, showPopups }) => {

    const [data, setData] = useState({ date: "", type:"" })
    ...
    const create = async () => {
        let response = await axios.post(`${baseUrl}/disable/create`, data)
        if (response.status) {
            getData()
    ...
    const del = async (id) => {
        let response = await axios.delete(`${baseUrl}/disable/delete/${id}`)
        if (response.status) {
            // getData()
    ...
    return (
        ...
        <div className='bg-white p-3 w-[90%] md:w-[50%] rounded-md overflow-auto h-[20rem]'>
            ...
            <div className='flex justify-between items-center w-[100%]'>
                <p>Unavialable Dates (Extras)</p>
                <ImCross className='cursor-pointer' onClick={() => { setShowPopups({ ...showPopups, disable: false }) }} />
            </div>
            ...
            <div className='lg:flex justify-between items-center mt-3 gap-x-3'>
                <input type="text" name='type' placeholder='Enter Extra Type' className='block w-[100%] lg:mt-0 mt-2 lg:flex-1 h-[2.3rem] rounded-md px-3 outline-none border border-gray-300' onChange={(e) => onChange(e)} />
