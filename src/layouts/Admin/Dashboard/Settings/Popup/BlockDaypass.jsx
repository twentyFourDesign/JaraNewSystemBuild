...
const BlockDaypass = ({ setShowPopups, showPopups }) => {

    const [data, setData] = useState({ date: "" })
    const [apiData, setapiData] = useState([])

    const getData = async () => {
        let response = await axios.get(`${baseUrl}/block/booking/get`)
    ...
    const create = async () => {
        let response = await axios.post(`${baseUrl}/block/booking/create`, data)
        if (response.status) {
            getData()
    ...
    const del = async (id) => {
        let response = await axios.delete(`${baseUrl}/block/booking/delete/${id}`)
        if (response.status) {
            // getData()
    ...
    return (
        ...
        <div className='bg-white p-3 w-[90%] md:w-[50%] rounded-md overflow-auto h-[20rem]'>
            ...
            <div className='flex justify-between items-center w-[100%]'>
                <p>Blocked Day Pass Booking Dates</p>
                <ImCross className='cursor-pointer' onClick={() => { setShowPopups({ ...showPopups, blocked: false }) }} />
            </div>
