...
const DiningExperience = ({ setShowPopups, showPopups }) => {
  const [data, setData] = useState({ title: "", price: "", typeOf: "" });
  const [apiData, setapiData] = useState([]);

  const getData = async () => {
    let response = await axios.get(`${baseUrl}/diningExperience/get`);
  ...
  const create = async () => {
    let response = await axios.post(`${baseUrl}/diningExperience/create`, data);
    if (response.status) {
      getData();
      toast.success("diningExperience Added");
    }
  };

  const update = async (id) => {
    let response = await axios.put(
      `${baseUrl}/diningExperience/update/${id}`,
      data
    );
    if (response.status) {
      getData();
      toast.success("diningExperience Updated");
    }
  };

  const del = async (id) => {
    let response = await axios.delete(
      `${baseUrl}/diningExperience/delete/${id}`
    );
    if (response.status) {
      // getData()
      toast.success("diningExperience Deleted");
    }
  };
    ...
    <div className="bg-white p-3 w-[90%] md:w-[80%] rounded-md overflow-auto h-[40rem]">
      <div className="flex justify-between items-center w-[100%]">
        ...
        <ImCross
          ...
          onClick={() => {
            setShowPopups({ ...showPopups, diningExperience: false });
          }}
        />
      </div>
          ...
          <>
            <div className="mt-4 flex justify-between overflow-auto items-center mb-2">
              ...
              <select
                name="typeOf"
                id="typeOf"
                defaultValue={i.typeOf}
                onChange={(e) => onChange(e)}
                className="w-[10rem] h-[2.3rem] rounded-md px-3 outline-none border border-gray-300"
              >
                <option value="lookout breakfast">lookout breakfast</option>
      ...
      <div className="lg:flex justify-between items-center mt-3 gap-x-3">
        ...
        <select
          ...
          className="w-[10rem] h-[2.3rem] rounded-md px-3 outline-none border border-gray-300"
        >
          <option value="lookout breakfast" selected>
            lookout breakfast
          </option>
          <option value="lookout lunch">lookout lunch</option>
          <option value="beachside dinner">beachside dinner</option>
