...
const DaypassExtension = ({ setShowPopups, showPopups }) => {
  const [data, setData] = useState({ title: "", price: "" });
  const [apiData, setapiData] = useState([]);

  const getData = async () => {
    let response = await axios.get(`${baseUrl}/daypassExtension/get`);
  ...
  const create = async () => {
    let response = await axios.post(`${baseUrl}/daypassExtension/create`, data);
    if (response.status) {
      getData();
      toast.success("daypassExtension Added");
    }
  };

  const update = async (id) => {
    let response = await axios.put(
      `${baseUrl}/daypassExtension/update/${id}`,
      data
    );
    if (response.status) {
      getData();
      toast.success("daypassExtension Updated");
    }
  };

  const del = async (id) => {
    let response = await axios.delete(
      `${baseUrl}/daypassExtension/delete/${id}`
    );
    if (response.status) {
      // getData()
      toast.success("daypassExtension Deleted");
    }
  };
    ...
    <div className="bg-white p-3 w-[90%] md:w-[80%] rounded-md overflow-auto h-[40rem]">
      <div className="flex justify-between items-center w-[100%]">
        ...
        <ImCross
          ...
          onClick={() => {
            setShowPopups({ ...showPopups, daypassExtension: false });
          }}
        />
      </div>
...
export default DaypassExtension;
