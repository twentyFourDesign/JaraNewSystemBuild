...
const ShowSub = ({ showRoom, data }) => {
  ...
  const tdStyle =
    "p-4 text-sm text-gray-600 whitespace-nowrap  border-2 border-[#E9EBED] tracking-wider";
  ...
  useEffect(() => {
    axios.get(`${baseUrl}/main/rooms/sub/get/${data._id}`).then((res) => {
      if (res.status) {
        setRooms(res.data);
      }
    });
  ...
  const onDelete = (id) => {
    axios.delete(`${baseUrl}/main/rooms/sub/delete/${id}`).then((res) => {
    ...
    <div className="bg-white w-[90%] p-2 rounded-md h-[20rem] overflow-scroll">
      ...
      {rooms?.length > 0 ? (
        ...
        <h1 className="flex justify-center items-center h-[100%] text-lg">
          NO ROOMS ADDED IN THIS CATEGORY
        </h1>
      )}
