...
const CreateSub = ({ showRoom, data }) => {
  ...
  const submitData = (e) => {
    ...
    if (
      ...
      axios
        .post(`${baseUrl}/main/rooms/sub/create`, {
          ...roomData,
          availableRoom: roomData.totalRoom,
        })
        .then((res) => {
          if (res.status) {
  ...
  return (
    <div className="bg-white w-[70%] sm:w-[40%] p-2 rounded-md">
      ...
      <input
        onChange={(e) => {
          setroomData({ ...roomData, totalRoom: e.target.value });
        }}
        type="number"
        defaultValue={1}
        disabled
        placeholder="Enter Total Room In This Category"
        className="w-[100%] h-[2.3rem] px-3 border-2 border-gray-300 mt-4 rounded-md outline-none"
      />
      <input
        onChange={(e) => {
          setroomData({ ...roomData, adults: e.target.value });
        }}
        type="number"
      ...
      <input
        onChange={(e) => {
          setroomData({ ...roomData, children: e.target.value });
        }}
        type="number"
      ...
      <input
        onChange={(e) => {
          setroomData({ ...roomData, toddler: e.target.value });
        }}
        type="number"
        placeholder="Enter Total Toodler"
        className="w-[100%] h-[2.3rem] px-3 border-2 border-gray-300 mt-4 rounded-md outline-none"
      />
      <input
        onChange={(e) => {
          setroomData({ ...roomData, infant: e.target.value });
        }}
        type="number"
        placeholder="Enter Total Infant"
        className="w-[100%] h-[2.3rem] px-3 border-2 border-gray-300 mt-4 rounded-md outline-none"
      />
