      ...
      <div className="p-5 w-[17rem] md:block hidden border-r-2 border-solid border-[#e6e7e9]">
        ...
        <div className="mt-10">
          {adminUser?.role === "superAdmin"
            ? navLinks?.map((item, index) => {
                return (
                  <Link
                    to={item.link}
                    key={index + 1}
                    className="flex gap-x-4 items-center cursor-pointer mb-5 text-[#828893] hover:text-[#68b2d7] hover:bg-[#eff6ff]  p-2 rounded-md"
                  >
            ...
            : navLinks
                ?.filter((item) => item.title !== "Accounts")
                .map((item, index) => {
                  return (
                    <Link
                      to={item.link}
              ...
              return (
                <Link
                  ...
                  className={`flex gap-x-4 items-center cursor-pointer ${
                    index !== 9 && "mb-3"
                  } text-[#828893] hover:text-[#68b2d7] hover:bg-[#eff6ff]  p-2 rounded-md`}
                >
                  <span className="text-lg">{item.icons}</span>
                  <p className="text-lg">{item.title}</p>
                </Link>
